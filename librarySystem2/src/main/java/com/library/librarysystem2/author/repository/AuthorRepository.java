...
public interface AuthorRepository extends JpaRepository<Authors, Integer> {
    ...
    @Query(value = " SELECT B.ISBN, B.Title, A.name, B.IsCheckedOut FROM Authors AS A JOIN Book_Authors AS BA ON A.authorID = BA.authorID JOIN Book AS B ON BA.ISBN = B.ISBN WHERE A.name like %:val% or B.Title like %:val%")
